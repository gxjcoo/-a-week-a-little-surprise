<template>
  <div class="">
    <div id="holder" :style="{ width: cw + 'px', height: ch + 'px' }"></div>
  </div>
</template>
    ...
    init() {
      function WaterRipple(element, settings) {
        ...
        function saveImageData() {
          // 在canvas中绘制图形
          //图片宽度 ==width
          //图片高度 == width*(imgh/imgw)
          //图片margin-top == width*(imgh/imgw)*某（暂时没计算）
        ...
        function init() {
          saveImageData();
          // 波幅数组初始化为0
          for (let i = 0; i < amplitude_size; i++) {
        ...
        function disturb(circleX, circleY) {
          ...
          circleX <<= 0;
          circleY <<= 0;
          let maxDistanceX = circleX + dropRadius,
            maxDistanceY = circleY + dropRadius;
          for (let y = circleY - dropRadius; y < maxDistanceY; y++) {
            for (let x = circleX - dropRadius; x < maxDistanceX; x++) {
        ...
        function renderRipple() {
          ...
          for (let y = 0; y < height; y++) {
            for (let x = 0; x < _width; x++) {
              ...
              let top = _ripple_map[_map_index - _width], // 上边的相邻点
              ...
              let old_amplitude = _last_map[i];
              _last_map[i] = amplitude;

              if (old_amplitude != amplitude) {
                deviation_x =
                  ((((x - _half_width) * amplitude) / _maxAmplitude) << 0) +
                  _half_width;
                deviation_y =
                  ((((y - _half_height) * amplitude) / _maxAmplitude) << 0) +
                  _half_height;
                ...
                _ripple_data[pixel_source + 1] =
                  _texture_data[pixel_deviation + 1];
                _ripple_data[pixel_source + 2] =
                  _texture_data[pixel_deviation + 2];
        ...
        function calculAmplitude(index, old_amplitude) {
          ...
          let top = ripple_map[index - width], // 上边的相邻点
          ...
          // 计算当前像素点下一时刻的振幅
          let amplitude = top + bottom + left + right;
          amplitude >>= 1;
      ...
      let settings = {
        image: this.img, //image path
        dropRadius: 4, //radius of the ripple
        width: this.cw, //width
      ...
      //init
      let waterRippleEffect = new WaterRipple(
